...
void Renderer::createInstancedDescriptorSets() {
    ...
    for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        vk::DescriptorBufferInfo globalBufferInfo{
            .buffer = *globalUboResources.uniformBuffers[i],
            .offset = 0,
            .range = sizeof(GlobalUBO)
        };

        vk::DescriptorBufferInfo instanceBufferInfo{
            .buffer = *instancedBufferResources.uniformBuffers[i],
            .offset = 0,
...
void Renderer::createInstancedBuffers() {
    vk::DeviceSize globalBufferSize = sizeof(GlobalUBO);
    globalUboResources.uniformBuffers.reserve(MAX_FRAMES_IN_FLIGHT);
    globalUboResources.uniformBuffersMemory.reserve(MAX_FRAMES_IN_FLIGHT);
    globalUboResources.uniformBuffersMapped.reserve(MAX_FRAMES_IN_FLIGHT);

    vk::DeviceSize instanceBufferSize = sizeof(InstanceData) * MAX_OBJECTS;
    instancedBufferResources.uniformBuffers.reserve(MAX_FRAMES_IN_FLIGHT);
    instancedBufferResources.uniformBuffersMemory.reserve(MAX_FRAMES_IN_FLIGHT);
    instancedBufferResources.uniformBuffersMapped.reserve(MAX_FRAMES_IN_FLIGHT);

    for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        vk::raii::Buffer gBuffer = nullptr;
        vk::raii::DeviceMemory gMemory = nullptr;
        createBuffer(globalBufferSize, vk::BufferUsageFlagBits::eUniformBuffer, vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent, gBuffer, gMemory);
        void* gMapped = gMemory.mapMemory(0, globalBufferSize);
        globalUboResources.uniformBuffers.emplace_back(std::move(gBuffer));
        globalUboResources.uniformBuffersMemory.emplace_back(std::move(gMemory));
        globalUboResources.uniformBuffersMapped.emplace_back(gMapped);

        vk::raii::Buffer iBuffer = nullptr;
        vk::raii::DeviceMemory iMemory = nullptr;
        createBuffer(instanceBufferSize, vk::BufferUsageFlagBits::eStorageBuffer, vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent, iBuffer, iMemory);
        void* iMapped = iMemory.mapMemory(0, instanceBufferSize);
        instancedBufferResources.uniformBuffers.emplace_back(std::move(iBuffer));
        instancedBufferResources.uniformBuffersMemory.emplace_back(std::move(iMemory));
        instancedBufferResources.uniformBuffersMapped.emplace_back(iMapped);
    }
}
